...
本次测试所用的矩阵主要来自于\cite{park2014sparsifying}和\cite{liuSyncFree2016}这两片文章中所用到的矩阵，这些矩阵都可以从MatrixMarket和弗罗里达大学稀疏矩阵集合中下载。本次测试所使用的矩阵如表\ref{MatrixSuite}所示。表中最后一列并行性的定义为并行性=矩阵的行数$ \div $最大level数，可以理解为平均每个level有多少的任务，其中并行性最好的是矩阵nlpkkt160，只有两个level，理论并行性最差的是af\_shell10和chipcool0。
...
\begin{table}[htbp]
    ...
    nlpkkt160     & 8,345,600     & 22,9518,112   & 2   & 4,172,800   \\
